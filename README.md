...
- **customAction(string $package, string $action, array $args=[], string $ip="127.0.0.1", int $port=8080):mixed**: Tells communicator server to run a custom action, returns the result of the action on success or false on failure.
